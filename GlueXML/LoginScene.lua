...
ModelList = {
	...
	-- Scene: 1
	--{1, -0.400, -10.700, -1.300, 0.000, 4.900, 0.204, _, 1, 1, 1, "Spells/spells/invisible.mdx", _, _},
	
	-- Scene: 1
	{1, -0.300, -9.800, 0.000, 0.000, 4.600, 0.125, _, 1, 1, 1, "Spells/spells/invisible.mdx", _, _}
}
...
local login_music_path = "sound/music/gluescreenmusic/wow_main_theme.mp3"		-- path to the music
...
--[[LoginScreenBackgroundNoble = LoginScene:CreateTexture(nil,"LOW")
	LoginScreenBackgroundNoble:SetPoint("CENTER",LoginScene)
	LoginScreenBackgroundNoble:SetTexture("Interface/GLUES/LOADINGSCREENS/Stormwind.blp")
	LoginScreenBackgroundNoble:SetSize(1400,800)]]
...
AnimLoginScreenBackgroundNoble = LoginScene:CreateTexture(nil,"LOW")
AnimLoginScreenBackgroundNoble:SetPoint("CENTER",LoginScene)
AnimLoginScreenBackgroundNoble:SetTexture("Interface/AnimBackgrounds/NobleLogin/0001.blp")
AnimLoginScreenBackgroundNoble:SetSize(1400,790)


local AnimFrameCount = 200
local currentFrame = 1
local ONUPDATE_INTERVAL = 0.03

local function UpdateAnimBack()
	local textureEnd = string.rep("0", (4-#tostring(currentFrame)))..currentFrame
	AnimLoginScreenBackgroundNoble:SetTexture("Interface/AnimBackgrounds/NobleLogin/"..textureEnd..".blp")
	currentFrame = currentFrame + 1
	if currentFrame > AnimFrameCount then
		currentFrame = 1
	end
end

local TimeSinceLastUpdate = 0
LoginScene:SetScript("OnUpdate", function(self, elapsed)
...
--[[
local fparent = CreateFrame("Frame","FParent",AccountLogin)
fparent:SetWidth(55)
fparent:SetHeight(55)
local mov = CreateFrame("MovieFrame","TestMovie",UIParent)
mov:SetWidth(1920)
mov:SetHeight(1080)
mov.resolution = 2
mov:SetPoint("CENTER", UIParent, "CENTER",0,0)
mov:SetWidth(1920)
mov:SetHeight(1080)

SetGlueScreen("movie");
local playSuccess, errorCode = mov:StartMovie("Interface\\Cinematics\\WOW_Intro_1024",0);
SetGlueScreen("movie");
local MainFrame =  CreateFrame("Button", "MA_MainFrame", mov)
MainFrame:SetWidth(150)
MainFrame:SetHeight(150)
MainFrame:SetPoint("CENTER", mov, "CENTER",0,0)
