...
ServerData.players_progressing = ServerData.players_progressing or {}
...
function SaveToClient(player,key,data)
	ServerData[key] = ServerData[key] or {}
	ServerData[key] = data

end

function SaveToClientInTable(player,tab,key,data)
	ServerData[tab] = ServerData[tab] or {}
	ServerData[tab][key] = ServerData[tab][key] or {}
	ServerData[tab][key] = data
end
function GetLevelProgressingFromServerData()
	local data = ServerData
	local result = {}
	result.players_progressing = {}
	for name,leveldata in pairs(data.players_progressing) do
		local progressing = {}
		progressing.level = 1
		progressing.class_name = "Без класса"
		if leveldata.level then
			progressing.level = tonumber(tostring(leveldata.level))
		end
		if leveldata.class_name then
			progressing.class_name = tostring(leveldata.class_name)
		end
		result.players_progressing[tostring(name)]= progressing
	end
...
function UnitLevel(unit)
	local a = {}
	a = deepcopy(GetLevelProgressingFromServerData())
	local level = OldUnitLevel(unit)
	if not UnitIsPlayer(unit) then
		return level
		
	end
	if a then
		if a.players_progressing then
			if a.players_progressing[UnitName(unit)] then
				level = a.players_progressing[UnitName(unit)].level
			end
		else
			return -1
		end
