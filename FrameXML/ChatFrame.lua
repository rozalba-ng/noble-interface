...
function GetColoredName(event, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10, arg11, arg12,postfix)
	...
	if ( info and info.colorNameByClass and arg12 ~= "" ) then
		...
		if ( englishClass ) then
			...
			return string.format("\124cff%.2x%.2x%.2x", classColorTable.r*255, classColorTable.g*255, classColorTable.b*255)..arg2..postfix.."\124r"
		end
	end
	
	return arg2..postfix;
end
local lastChatBoxInfo = {}
lastChatBoxInfo.charName = ""
lastChatBoxInfo.type = ""
lastChatBoxInfo.timeInSec = 0
function ChatFrame_MessageEventHandler(self, event, ...)
	if ( strsub(event, 1, 8) == "CHAT_MSG" ) then
		...
		local addonInfo = {}
		if arg12 ~= "" then
			addonInfo = CHARS_INFO_LIST[arg2]
			if addonInfo then
				if addonInfo.postfix ~= nil then
					if addonInfo.postfix ~= "" then
						postfix = " " .. FullEscChar(string.utf8sub(addonInfo.postfix,0,15))
					end
				end
			end
		end
		
		local coloredName = GetColoredName(event, arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8, arg9, arg10, arg11, arg12,postfix);
		...
		elseif ( type == "BN_INLINE_TOAST_CONVERSATION" ) then
			...
			if(strlen(arg6) > 0) then
				if ( arg6 == "GM" ) then
					...
					pflag = "|TInterface\\ChatFrame\\UI-ChatIcon-Blizz.blp:0:2:1.5:-2|t ";
			...
			local showLink = 1;
			if ( strsub(type, 1, 7) == "MONSTER" or strsub(type, 1, 9) == "RAID_BOSS" or (lastChatBoxInfo[self:GetName()] ~= nil and lastChatBoxInfo[self:GetName()].charName == arg2 and lastChatBoxInfo[self:GetName()].type == type and ((GetTime()) - lastChatBoxInfo[self:GetName()].timeInSec) < 0.4)) then
				showLink = nil;
			else
				arg1 = gsub(arg1, "%%", "%%%%");
			end
			...
			if ( (strlen(arg3) > 0) and (arg3 ~= "Universal") and (arg3 ~= self.defaultLanguage) ) then
				...
				if ( not showLink or strlen(arg2) == 0 ) then
					if lastChatBoxInfo[self:GetName()] ~= nil and lastChatBoxInfo[self:GetName()].charName == arg2 and lastChatBoxInfo[self:GetName()].type == type and ((GetTime()) - lastChatBoxInfo[self:GetName()].timeInSec) < 0.4 then
						body = format("    "..arg1, pflag..arg2, arg2);
					else
			...
			lastChatBoxInfo[self:GetName()] = {}
			lastChatBoxInfo[self:GetName()].charName = arg2
			lastChatBoxInfo[self:GetName()].type = type
			lastChatBoxInfo[self:GetName()].timeInSec = GetTime()
...
function SendToPlayersMyPostfix()
	SendAddonMessage("CALL_TO_SHARE_POSTFIX", Compressor:Serialize(CHAR_INFO.postfix),"WHISPER",UnitName("player"))
end
...
function ChatEdit_OnTextChanged(self, userInput)
	...
	if strlen(text) > 50 then
		SAVED_CHATBOX = self:GetText()	
	end
	
	if text == "/revert" and SAVED_CHATBOX ~= nil then
		self:SetText(SAVED_CHATBOX)
	end
    if (self:IsShown() and text:len() > 0 and firstChar ~= "/" and firstChar ~= ".") then
        self.IsTyping = true;
        if chatType == "SAY" then
            SendAddonMessage("PLAYER_CHAT_TYPING", "SAY", "WHISPER", UnitName("player"));
        elseif chatType == "YELL" then
            SendAddonMessage("PLAYER_CHAT_TYPING", "YELL", "WHISPER", UnitName("player"));
        elseif chatType == "EMOTE" then
            SendAddonMessage("PLAYER_CHAT_TYPING", "EMOTE", "WHISPER", UnitName("player"));
        end
    else
        SendAddonMessage("PLAYER_CHAT_TYPING", "STOP", "WHISPER", UnitName("player"));
    end
