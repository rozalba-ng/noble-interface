...
local ROLL_STATS = {0,0,0,0,0,0,0}
...
function PaperDollFrame_OnEvent (self, event, ...)    
    ...
    elseif ( event == "CHAT_MSG_ADDON" ) then
        if ( arg4 == UnitName("player") and arg1 == "PLAYER_ROLESTAT_UPDATE") then
            index = 1;
            for stat in string.gmatch(arg2, "([^,]+)") do
                ROLL_STATS[index] = stat;
                index=index+1;
            end
            PaperDollFrame_UpdateStats(self);
        end
...
function PaperDollFrame_SetRollStat(statFrame, statIndex)
	local label = _G[statFrame:GetName().."Label"];
	local text = _G[statFrame:GetName().."StatText"];
	local statName = _G["SPELL_ROLLSTAT"..statIndex.."_NAME"];
	label:SetText(format(STAT_FORMAT, statName));
	
	-- Set the tooltip text
	local tooltipText = HIGHLIGHT_FONT_COLOR_CODE..format(PAPERDOLLFRAME_TOOLTIP_FORMAT, statName).." ";
    
    text:SetText(ROLL_STATS[statIndex]);
    statFrame.tooltip = tooltipText..ROLL_STATS[statIndex]..FONT_COLOR_CODE_CLOSE;
	
	statFrame.tooltip2 = _G["DEFAULT_ROLLSTAT"..statIndex.."_TOOLTIP"];
	statFrame.tooltip2 = format(statFrame.tooltip2, ROLL_STATS[statIndex]);
...
function UpdatePaperdollStats(prefix, index)
	...
	if ( index == "PLAYERSTAT_BASE_STATS" ) then
        ...
        PaperDollFrame_SetRollStat(stat1, 1);
		PaperDollFrame_SetRollStat(stat2, 2);
		PaperDollFrame_SetRollStat(stat3, 3);
		PaperDollFrame_SetRollStat(stat4, 4);
		PaperDollFrame_SetRollStat(stat5, 5);
		PaperDollFrame_SetRollStat(stat6, 6);
        PaperDollFrame_SetRollStat(stat7, 7);         
